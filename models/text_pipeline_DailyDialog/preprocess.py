...
def load_lines(path):
    if not os.path.exists(path):
        raise FileNotFoundError(
            ...
            "Place DailyDialog files inside the data/ folder."
        )
...
def group_split(df):
    ...
    Same dialogue_id will not appear in more than one split.
    Final ratio: 70% train, 15% validation, 15% test.
    """
    ...
    train_idx, temp_idx = next(
        splitter_1.split(df, groups=df["dialogue_id"])
    )
    ...
    val_idx, test_idx = next(
        splitter_2.split(temp_df, groups=temp_df["dialogue_id"])
    )
...
def main():
    ...
    df = df.drop_duplicates(
        subset=["dialogue_id", "utterance_id", "text", "emotion"]
